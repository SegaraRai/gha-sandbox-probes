...
`gha-sandbox-probes` runs untrusted or high-risk CI commands inside a hardened
Docker sandbox, verifies that important GitHub Actions attack surfaces are not
visible, and only then runs your command in a sanitized environment.

Use it for compatibility tests, dependency-graph tests, package-manager
experiments, or other CI jobs where third-party code may execute. It is designed
for attacks that try to steal CI credentials, read runner process memory, poison
caches, reach container runtimes, or exfiltrate cloud and package credentials.

There are two entry points:

- A composite GitHub Action that starts the Docker sandbox, runs the probes, and
  runs your command from a copied workspace.
- A standalone shell probe that can be pinned by commit SHA and executed inside
  a sandbox you create yourself.

The probes fail closed. If a required sandbox boundary is broken, the script
exits non-zero.
...
The default network mode is `none`. If your sandboxed command must install
dependencies or otherwise use outbound network, opt in explicitly:
...
Pin the raw URL to a full-length commit SHA and run the script inside the
...
Running the standalone script directly on a normal runner shell is expected to
fail checks such as `container-marker` and `zero-capabilities`; it is meant to
evaluate a sandbox, not to make the host runner safe by itself.
...
By default, the action inherits only a small allowlist of non-sensitive variables
that preserve terminal behavior, locale, timezone, and reproducible build
settings:

- Color and terminal: `TERM`, `COLORTERM`, `NO_COLOR`, `FORCE_COLOR`,
  `CLICOLOR`, `CLICOLOR_FORCE`
- Tool color controls: `CARGO_TERM_COLOR`, `CMAKE_COLOR_DIAGNOSTICS`,
  `PY_COLORS`, `PYTHON_COLORS`
...
package names. Pass non-sensitive values with `env`, or inherit specific names
with `inherit-env`, when you understand the exposure.

GitHub Actions runtime, cache, OIDC, file-command, package-token, SSH-agent, and
similar CI credential variables are blocked from normal `env` and `inherit-env`.
To expose one anyway, use `unsafe-env` or `unsafe-inherit-env`; these inputs are
intentionally verbose because the sandboxed command can read and exfiltrate
those values.

Explicitly passed or inherited variable names are registered in
`GHA_SANDBOX_ALLOWED_ENV_NAMES` before the probe runs. The probe excludes those
names from environment-name and URL-userinfo checks, but still checks structural
sandbox boundaries such as runner process visibility, container runtime sockets,
credential files, metadata token endpoints, capabilities, and writable mounts.
...
image entrypoint cannot run before the probes. Minimal images such as distroless
images, Alpine without Bash, or slim images missing these utilities will fail
before the sandboxed command runs.

The host workspace is mounted read-only at `/workspace`, then copied to
...
The action intentionally does not know how to install project dependencies. Keep
setup separate from the sandbox runner. For example:
...
The probes are designed for CI jobs where the payload may execute arbitrary
code, including package install hooks, dependency tests, third-party actions, or
tools pulled from public ecosystems.

They are intended to catch or prevent exposure of:

- GitHub Actions runtime, cache, results, and OIDC credentials.
- Container runtime access through Docker, containerd, Podman, CRI-O, BuildKit,
  or `DOCKER_HOST`.
...
This project is not a complete sandbox. It is a set of runtime assertions that
help verify that a separate sandboxing mechanism is actually in effect.

You are still responsible for:

- Pinning this action, the sandbox image, setup scripts, and third-party actions
  to immutable references where practical.
- Using `actions/checkout` with `persist-credentials: false` unless the job
  needs authenticated Git operations.
- Keeping secrets, write tokens, OIDC permissions, publish credentials, and
  trusted caches out of jobs that run untrusted code.
- Opting into `network: bridge` only when the sandboxed command needs outbound
  network.
- Treating anything passed through `unsafe-env`, `unsafe-inherit-env`, or
  `disable-checks` as an explicit risk acceptance.
- Reviewing package-manager install behavior. This action can isolate install
  scripts, but it does not decide which dependencies are trustworthy.
...
- Keep untrusted code out of `pull_request_target` and `workflow_run` jobs with
  secrets, write tokens, OIDC, or privileged cache access.
...
- `GHA_SANDBOX_ALLOWED_ENV_NAMES`: optional comma or whitespace separated list
  of environment variable names that should be treated as explicitly
  risk-accepted by the environment probe.
